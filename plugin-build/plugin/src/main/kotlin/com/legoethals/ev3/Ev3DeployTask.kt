...
abstract class Ev3DeployTask @Inject constructor(private val sshServiceProvider: SshServiceProvider) : DefaultTask() {
    ...
    private fun checkDeployedArtifactUpToDate(): Boolean {
        return sshServiceProvider.create().use {
            val currentMd5File = inputArtifactMd5.get().asFile
            val currentArtifactMd5 = currentMd5File.readText()
            val remoteArtifactMd5FilePath =
                artifactChecksumDestinationDir.get().trimEnd('/') + "/" + currentMd5File.name
            val remoteArtifactMd5 = it.downloadFileContents(remoteArtifactMd5FilePath)
            if (remoteArtifactMd5 == null) {
                false
            } else {
                ...
                currentArtifactMd5 == remoteArtifactMd5
            }
        }
    }
